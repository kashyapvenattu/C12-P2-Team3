...
const App = () => {
  ...
  const [filteredConstructionMarkers, setFilteredConstructionMarkers] =
    useState([]);
          ...
          for (let i = 0; i < response.routes.length; i++) {
            ...
            const polyline = new google.maps.Polyline({
              path: google.maps.geometry.encoding.decodePath(
                response.routes[i].overview_polyline
              ),
            });
            const newDirections = new google.maps.DirectionsRenderer({
              map: map,
              directions: response,
              routeIndex: i,
            });
        ...
        <div className="navigation-bar">
          ...
          <div className="user-and-login">
            ...
            <IfLoggedIn>
              <p>
                {" "}
                Welcome <b>{loggedInUser?.username} </b>
              </p>
            </IfLoggedIn>
          </div>
        </div>
        ...
        <div className="rankCard" onClick={toggleRank}>
          {rankJson.map((rankObject, index) => (
            <div className="rankCardItem" key={index}>
              <p className="para">
                <b>Route Name: {rankObject.name}</b>
              </p>
