...
async function main() {
    ...
    var ConstructionDetours = mongoose.model('ConstructionDetours', constructionSchema);
    ...
    //const promises = []
    try {
        const routeResponse = await fetch('https://data.calgary.ca/resource/hpnd-riq4.json');
        const routeData = await routeResponse.json();
        //promises.push(routeResponse.json());

        const stopsResponse = await fetch('https://data.calgary.ca/resource/muzh-c9qc.json')
        const stopsData = await stopsResponse.json();
        //promises.push(stopsResponse.json());

        //await Promise.all(promises)
        const constructionResponse = await fetch('https://data.calgary.ca/resource/w8zq-79bq.json')
        const constructionData = await constructionResponse.json();

        const trafficIncidentsResponse = await fetch('https://data.calgary.ca/resource/4jah-h97u.json')
        const trafficIncidentsData = await trafficIncidentsResponse.json();

        const snowAndIceResponse = await fetch('https://data.calgary.ca/resource/4v8s-3kss.json')
